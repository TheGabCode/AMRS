...
public class Scheduler{
	...
	public void start(){
		while(instructions.size() != 0 || readyQueue.size() != 0 || queue.get("W") != null){
			...
			if(this.queue.get("M") != null)		this.queue.get("M").memory(special_registers, clock_cycle);
			if(this.queue.get("E") != null)		this.queue.get("E").execute(registers, clock_cycle);
			if(this.queue.get("D") != null)		this.queue.get("D").decode(special_registers, clock_cycle);
			if(this.queue.get("F") != null)		this.queue.get("F").fetch(special_registers, clock_cycle);
			...
			if(this.stall.size() != 0 && this.clock_cycle == this.stall.getFirst()+5){
				...
				this.stall.pop();
				System.out.print("Stalled instruction continue\n");
			}
